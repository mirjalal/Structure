...
import az.talmir.app.structure.core.features.auth.AuthResponse
...
class JsonUnitTest : KoinTest {
    ...
    fun `test string parsing to AuthRemoteResponse without refresh_token returns true`() {
        ...
        val obj = json.decodeFromString<AuthResponse>(strJson)
        assertEquals(AuthResponse(access_token = "blahblah"), obj)
    }
    ...
    fun `test string parsing to AuthRemoteResponse with refresh_token returns true`() {
        ...
        val obj = json.decodeFromString<AuthResponse>(strJson)
        assertEquals(AuthResponse(access_token = "blahblah", refresh_token = "qweqwe"), obj)
    }

    @Test
    fun `test AuthRemoteResponse parsing with nullable prop value returns true`() {
        val obj = AuthResponse(access_token = "blah_blah", refresh_token = null)
    ...
    fun `test AuthRemoteResponse parsing without nullable prop value returns true`() {
        val obj = AuthResponse(access_token = "blah_blah", refresh_token = "qwe_qwe")
        val strJson = json.encodeToString(obj)
    ...
    fun `test parsing unknown keys returns true`() {
        ...
        val obj = json.decodeFromString<AuthResponse>(strJson)
        assertEquals(AuthResponse(access_token = "blahblah", refresh_token = null), obj)
    }
}
